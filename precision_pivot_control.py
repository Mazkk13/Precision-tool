...
class controls():            
    ...
    def findAndAnnotateEdge(self, object, objectName, queryPrecision):
        ...
        '''
        edgeToVertex = cmds.polyInfo(object, edgeToVertex=True)[0].split(':')[1].strip()  
        vertexNum  = [edgeToVertex.split('    ')[i] for i in range(2)]
        vertices = [objectName + '.vtx[' + str(vertexNum[i]) + ']' for i in range(2)]
        vertexPositions = [cmds.pointPosition(vertices[i]) for i in range(2)]
        edgeMidpoint = [(vertexPositions[0][i] + vertexPositions[1][i])/2 for i in range(3)]
        rounded = [round(edgeMidpoint[i], queryPrecision) for i in range(3)]                    
        cmds.annotate(objectName, p=[edgeMidpoint[0], edgeMidpoint[1], edgeMidpoint[2]], text=str(rounded))
        cmds.select(object)
    ...
    def findAndAnnotateFaceNormal(self, object, objectName, queryPrecision):
        ...
        '''
        facePos = cmds.polyInfo(object, fn=True)[0].split(':')[1].strip().split(' ')
        rounded = [round(float(facePos[i]), queryPrecision) for i in range(3)]
        cmds.annotate(objectName, p=[rounded[0], rounded[1], rounded[2]], text=str(rounded))
        cmds.select(object)
